    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-[#faf6ec]/95 backdrop-blur-sm shadow-lg shadow-black/5 border-b border-stone-200' : 'bg-transparent'}`}>
      <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
        <a href="#" className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg bg-amber-700 flex items-center justify-center font-bold text-white text-sm">
            SJ
          </div>
        ...
        <div className="hidden md:flex items-center gap-8">
          {links.map(l => (
            <button key={l.label} onClick={() => scrollTo(l.id)} className="text-stone-500 hover:text-amber-700 transition-colors text-sm font-medium">
              {l.label}
            </button>
          ))}
          <a
      ...
      {menuOpen && (
        <div className="md:hidden bg-[#faf6ec] border-t border-stone-200 px-6 py-4 flex flex-col gap-4">
          {links.map(l => (
            <button key={l.label} onClick={() => { scrollTo(l.id); setMenuOpen(false) }} className="text-stone-700 hover:text-amber-700 transition-colors text-sm font-medium text-left">
              {l.label}
            </button>
          ))}
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-2 text-stone-700 hover:text-amber-700 transition-colors text-sm font-medium"
          >
            <HelpCircle size={16} /> Visitor Handbook
          </a>
          <button onClick={() => { setMenuOpen(false); setHireMeOpen(true) }} className="bg-amber-700 text-white text-sm font-semibold px-4 py-2 rounded-lg text-center">
            Hire Me
          </button>
        </div>
      )}
