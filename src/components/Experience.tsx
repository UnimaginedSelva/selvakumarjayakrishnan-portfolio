    ...
    <section id="experience" className="py-24 px-6 bg-stone-50">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="relative">
          {/* Timeline line */}
          <div className="absolute left-4 md:left-8 top-0 bottom-0 w-px bg-gradient-to-b from-amber-700 via-stone-300 to-transparent" />

          <div className="space-y-12">
            {experience.map((item, i) => (
              <div key={i} className="relative pl-12 md:pl-20">
                {/* Timeline dot */}
                <div className="absolute left-0 md:left-4 top-1 w-8 h-8 bg-white border-2 border-amber-700 rounded-full flex items-center justify-center">
                  ...
                  <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3 mb-4">
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <span className="text-amber-700 font-semibold text-sm">{item.company}</span>
                    ...
                    <div className="flex flex-col items-start md:items-end gap-1 shrink-0">
                      ...
                      {item.award && item.award.split('|').map(a => a.trim()).filter(Boolean).map(a => (
                        <span key={a} className="flex items-center gap-1 text-xs bg-amber-100 text-amber-800 border border-amber-200 px-2 py-0.5 rounded-full">
                          <Award size={10} />
                          {a}
                        </span>
                      ))}
                  ...
                  <ul className="space-y-2">
                    {item.highlights.map((h, j) => (
                      <li key={j} className="flex gap-3 text-stone-600 text-sm leading-relaxed">
                        <span className="text-amber-700 mt-1.5 shrink-0">›</span>
                        <span>{h}</span>
                      </li>
                    ))}
                  </ul>
