...
// Same prompt/logic as functions/api/job-intel.ts (Claude version) — independent alongside
// build so the two can diverge or be compared without cross-coupling.
...
- Also published: ASCEND™, EMBED™, BRIDGE™, FORGED™, TRUST™, STEWARD™

**CRITICAL — How to position the frameworks in interview context:**
These frameworks were developed based on Selva's 19 years of experience — they were NOT used during his Dell career. In STAR answers, Selva references what he actually did at Dell (ADKAR-based methodology). The frameworks are then positioned as thought leadership: "Based on 19 years of doing this, I identified consistent gaps in how ADKAR gets applied in practice. That's what led me to develop TRANSFORM™/OPERATE™ — they solve the last-mile problems that ADKAR practitioners consistently hit." This is honest, credible, and differentiating.
...
Return ONLY valid JSON matching this exact shape. No markdown, no explanation, no code blocks.

{
  ...
  "fitAssessment": [ { "requirement": "Requirement from JD (keep concise)", "score": <1-5>, "label": "Very Strong" | "Strong" | "Moderate" | "Gap" | "Critical Gap", "evidence": "Specific Selva evidence that maps to this requirement" } ],
  "talkingPoints": [ { "angle": "Short label for this talking point (e.g. Opening Hook, Scale Credibility, AI Expertise)", "situation": "STAR - Situation: what was happening at Dell", "task": "STAR - Task: what Selva was responsible for", "action": "STAR - Action: what Selva specifically did using ADKAR-based methodology", "result": "STAR - Result: specific measurable outcome", "linkToJD": "How this directly maps to a requirement in this JD" } ],
  "frameworkAngles": [ { "jdRequirement": "The JD requirement this angle addresses", "starHook": "The real Dell achievement to anchor the STAR answer", "bridgeLine": "The transition line: 'Based on that experience, I observed...'", "frameworkPositioning": "How TRANSFORM™ or OPERATE™ addresses the gap identified — which specific stage and why", "forwardApplication": "How Selva would apply this in the new role specifically" } ],
  "questionsToAsk": [ { "question": "The question to ask", "intent": "Why this question signals strategic thinking" } ],
  "watchOuts": [ { "gap": "Potential gap or tough question", "preparedAnswer": "How to address it honestly and confidently" } ]
}`;
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  try {
    ...
    const response = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/gemini-flash-latest:generateContent?key=${context.env.GEMINI_API_KEY}`,
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          systemInstruction: { parts: [{ text: SYSTEM_PROMPT }] },
          contents: [{ role: 'user', parts: [{ text: `Generate an interview preparation brief for this role:\n\n${userMessage}` }] }],
          generationConfig: {
            responseMimeType: 'application/json',
            // gemini-flash-latest spends part of the token budget on internal reasoning
            // before writing the JSON, so this needs real headroom above the larger
            // job-intel JSON body.
    ...
    if (!response.ok) {
      ...
      return new Response(JSON.stringify({ error: `Service unavailable (Gemini ${response.status}): ${errText.slice(0, 300)}` }), {
        status: 502,
        headers: { 'Content-Type': 'application/json' },
      });
    }
    ...
    if (!text) {
      return new Response(JSON.stringify({ error: 'Empty response from Gemini. Please try again.' }), {
        status: 502,
        headers: { 'Content-Type': 'application/json' },
      });
    }
