...
// Same assessment logic/prompt as functions/api/analyze.ts (Claude version) — kept as an
// independent alongside build so the two can diverge or be compared without cross-coupling.
...

{
  ...
  "adkar": {
    "awareness": { "score": <1-5>, "label": "Very Low" | "Low" | "Medium" | "High" | "Very High", "rationale": "2-3 sentence assessment based on the brief", "keyGap": "The single most critical gap in this dimension" },
    "desire": { "score": <1-5>, "label": "...", "rationale": "...", "keyGap": "..." },
    "knowledge": { "score": <1-5>, "label": "...", "rationale": "...", "keyGap": "..." },
    "ability": { "score": <1-5>, "label": "...", "rationale": "...", "keyGap": "..." },
    "reinforcement": { "score": <1-5>, "label": "...", "rationale": "...", "keyGap": "..." }
  },
  "risks": [ { "title": "Risk title", "severity": "High" | "Medium" | "Low", "description": "2 sentence description of the risk and its impact" } ],
  "frameworkRecommendations": {
    "awareness": { "framework": "TRANSFORM™" | "OPERATE™" | "Both", "stages": ["Stage name — brief description"], "actions": ["Specific action 1", "Specific action 2"] },
  ...
  "priorityActions": ["Immediate action 1", "Immediate action 2", "Immediate action 3"]
}`;
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  try {
    ...
    const response = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/gemini-flash-latest:generateContent?key=${context.env.GEMINI_API_KEY}`,
      {
        ...
        body: JSON.stringify({
          ...
          contents: [{ role: 'user', parts: [{ text: `Assess the change readiness for this project:\n\n${brief}` }] }],
          generationConfig: {
            responseMimeType: 'application/json',
            // gemini-flash-latest spends part of the token budget on internal reasoning
            // before writing the JSON, so this needs real headroom above the ~2.5k the
            // JSON body itself takes.
    ...
    if (!response.ok) {
      ...
      return new Response(JSON.stringify({ error: `Assessment service unavailable (Gemini ${response.status}): ${errText.slice(0, 300)}` }), {
        status: 502,
        headers: { 'Content-Type': 'application/json' },
      });
    }
    ...
    return new Response(JSON.stringify({ error: `Failed to process assessment: ${err instanceof Error ? err.message : 'Unknown error'}` }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
    });
  }
